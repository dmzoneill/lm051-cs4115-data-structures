...
class AVL
{
	protected:
		...
		{
			public:
				AVLNode(KeyT key, ItemT item) :	m_Balance(0), m_Depth(0), m_Key(key), m_Data(item), m_pLeft(0), m_pRight(0){}
	...
	public:
		...
		*/
		...
		public:
			...
			Iterator(AVL<KeyT, ItemT>* pTree) : mpAVL(pTree)
			{
				KeyT key;
				ItemT item;
				GetFirst(key, item);
			...
			bool GetCurrent(KeyT& key, ItemT& item)
			{
				if (mpCurr)
				{
					key = mpCurr->m_Key;
			...
			bool GetFirst (KeyT& key, ItemT& item)
			{
				...
				{
					AVLNode<KeyT, ItemT>* pCurr = mpAVL->m_pRoot;
					AVLNode<KeyT, ItemT>* pPrev = pCurr;
					while (pCurr)
					{
						pPrev = pCurr;
			...
			bool GetNext  (KeyT& key, ItemT& item)
			{
				...
				{
					AVLNode<KeyT, ItemT>* pCurr = mpCurr->m_pRight;	// start looking to the right
					...
					while (true)	// this while forces a traversal as far left as possible
					{
						...
						else	// backtrack
						{
							...
							{
								AVLNode<KeyT, ItemT>* pCandidate = mTraversalStack[--mTraversalStackIndex];
			...
			bool Find     (KeyT  key, ItemT& item)
			{
				...
				while (true)
				{
					AVLNode<KeyT, ItemT>* pPushMe = pCurr;
		...
		protected:
			AVL* mpAVL;		
			AVLNode<KeyT, ItemT>*	mTraversalStack[kMaxAVLDepth];
			short mTraversalStackIndex;
			AVLNode<KeyT, ItemT>*	mpCurr; // for iteration
		};
